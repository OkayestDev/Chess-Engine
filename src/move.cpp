...
namespace Chess {
    ...
    void undo_move(Board& board, Boardhistory& ml) {
        if(!ml.size())
            moveSeturn;
                ...
                switch(board.getSquare(from).piece) {
                    case Piece::kingCastle:
                        ...
                        if(from == 25 && board.getSquare(24).piece == Piece::none && board.getSquare(23).piece == Piece::none && board.getSquare(22).piece == Piece::none && board.getSquare(21).piece == Piece::rookCastle)
                            moveSet.pushBack(Move(from, 23));
                    // fallthrough
                    case Piece::king:
                        for(auto to : {from-11, from-10, from-9,from-1, from+1, from+9, from+10,from+11}) {
                            if(board.getSquare(to).pieceColor == turn || board.getSquare(to).piece == Piece::out_of_board)
                                continue;
                            moveSet.pushBack(Move(from, to));
                        }
                        break;
                    case Piece::pawn:
                        if(board.getSquare(from+pawn_dir*11).pieceColor == enemy_color)
                            moveSet.pushBack(Move(from, from+pawn_dir*11));
                        if(board.getSquare(from+pawn_dir*9).pieceColor == enemy_color)
                            moveSet.pushBack(Move(from, from+pawn_dir*9));
                        if(board.getSquare(from+pawn_dir*10).piece == Piece::none)
                            moveSet.pushBack(Move(from, from+pawn_dir*10));
                        if(board.getSquare(from+pawn_dir*10).piece == Piece::none && board.getSquare(from+pawn_dir*20).piece == Piece::none){
                            size_t row = from/10;
                            if((row == 3 && pawn_dir == 1) || (row == 8 && pawn_dir == -1))
                                moveSet.pushBack(Move(from, from+pawn_dir*20));
                        }
                        if(board.getSquare(from-1).piece == Piece::pawnEnPassant && pawn_dir == -1)
                            moveSet.pushBack( Move(from, from-11) );
                        if(board.getSquare(from+1).piece == Piece::pawnEnPassant && pawn_dir == -1)
                            moveSet.pushBack( Move(from, from-9) );
                        if(board.getSquare(from-1).piece == Piece::pawnEnPassant && pawn_dir == 1)
                            moveSet.pushBack( Move(from, from+9) );
                        if(board.getSquare(from+1).piece == Piece::pawnEnPassant && pawn_dir == 1)
                            moveSet.pushBack( Move(from, from+11) );
                    ...
                    case Piece::rookCastle:
                        for(int dd : {1,-1,10,-10})
                            for(int d=dd; board.getSquare(from+d).pieceColor != board.getSquare(from).pieceColor && board.getSquare(from+d).piece != Piece::out_of_board ;d+=dd) {
                                moveSet.pushBack(Move(from, from+d));
